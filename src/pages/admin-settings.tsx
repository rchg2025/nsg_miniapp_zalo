...
function AdminSettingsPage() {
  ...
  const handleSaveSettings = () => {
    if (settings) {
      ...
      setHasChanges(false);
      alert('C�i đặt đ� được lưu th�nh c�ng!');
    }
  };

  const handleResetSettings = () => {
    if (confirm('Bạn c� chắc chắn muốn kh�i phục c�i đặt mặc định?')) {
      loadSettings();
      setHasChanges(false);
    }
  };
  ...
  if (!settings) {
    return (
      <Page className="page-with-header bg-gray-50">
        <Header 
          title="C�i đặt hệ thống" 
          showBackIcon={true}
          className="bg-blue-600 text-white"
        />
        <Box className="p-4">
          <Text>Đang tải c�i đặt...</Text>
        </Box>
      </Page>
    );
  }
    ...
    <Page className="page-with-header bg-gray-50">
      ...
      <Box className="p-4 space-y-6">
        ...
        {hasChanges && (
          <Box className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
            <Box className="flex justify-between items-center">
              <Text className="text-yellow-800">C� thay đổi chưa được lưu</Text>
        ...
        <Box className="bg-white p-4 rounded-lg">
          <Text.Title className="mb-4">🏫 Th�ng tin trường</Text.Title>
          <Box className="space-y-4">
            <Box>
              <Text className="mb-2 font-medium">T�n trường</Text>
              <Input
                value={settings.general.schoolName}
                onChange={(e) => handleSettingChange('general', 'schoolName', e.target.value)}
                placeholder="T�n đầy đủ của trường"
              />
            </Box>
            
            <Box className="grid grid-cols-2 gap-3">
              <Box>
                <Text className="mb-2 font-medium">M� trường</Text>
                <Input
        ...
        {/* Admission Settings */}
        <Box className="bg-white p-4 rounded-lg">
          <Text.Title className="mb-4">🎓 C�i đặt tuyển sinh</Text.Title>
          <Box className="space-y-4">
            <Box className="flex items-center justify-between">
              <Text className="font-medium">Mở đăng k� tuyển sinh</Text>
              <Switch
            ...
            <Box className="grid grid-cols-2 gap-3">
              <Box>
                <Text className="mb-2 font-medium">Ng�y bắt đầu</Text>
                <Input
              ...
              <Box>
                <Text className="mb-2 font-medium">Ng�y kết th�c</Text>
                <Input
            ...
            <Box>
              <Text className="mb-2 font-medium">Hồ sơ y�u cầu (c�ch nhau bằng dấu phẩy)</Text>
              <Input.TextArea
        ...
        {/* App Settings */}
        <Box className="bg-white p-4 rounded-lg">
          <Text.Title className="mb-4">📱 C�i đặt ứng dụng</Text.Title>
          <Box className="space-y-4">
            <Box className="grid grid-cols-2 gap-3">
              <Box>
              ...
              <Box>
                <Text className="mb-2 font-medium">Ng�n ngữ</Text>
                <Select
            ...
            <Box>
              <Text className="mb-3 font-medium">Th�ng b�o</Text>
              <Box className="space-y-3">
                <Box className="flex items-center justify-between">
                  <Text>Th�ng b�o đẩy</Text>
                  <Switch
            ...
            <Box>
              <Text className="mb-3 font-medium">T�nh năng</Text>
              <Box className="space-y-3">
                <Box className="flex items-center justify-between">
        ...
        {/* Security Settings */}
        <Box className="bg-white p-4 rounded-lg">
          <Text.Title className="mb-4">🔒 C�i đặt bảo mật</Text.Title>
          <Box className="space-y-4">
            <Box className="grid grid-cols-2 gap-3">
              <Box>
                <Text className="mb-2 font-medium">Thời gian phi�n (ph�t)</Text>
                <Input
                  type="number"
            ...
            <Box className="flex items-center justify-between">
              <Text className="font-medium">Bắt buộc quan t�m OA</Text>
              <Switch
            ...
            <Box>
              <Text className="mb-3 font-medium">Ch�nh s�ch mật khẩu Admin</Text>
              <Box className="space-y-3">
                <Box>
                  <Text className="mb-2">Độ d�i tối thiểu</Text>
                  <Input
                    type="number"
                ...
                <Box className="flex items-center justify-between">
                  <Text>Y�u cầu k� tự đặc biệt</Text>
                  <Switch
